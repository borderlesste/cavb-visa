...
export const runMigrations = async () => {
    const connection = await pool.getConnection();
    try {
    const dbName = (process.env.DB_DATABASE || process.env.DB_NAME) as string;
        ...
        const [rows]: any = await connection.execute(
            `SELECT COUNT(*) AS cnt FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'users' AND COLUMN_NAME = 'fullName'`,
            [dbName]
        );
        if (rows[0].cnt === 0) {
        ...
        const [appCols]: any = await connection.execute(
            `SELECT COUNT(*) AS cnt FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'applications' AND COLUMN_NAME = 'visa_type'`,
            [dbName]
        );
        if (appCols[0].cnt === 0) {
        ...
        const [appStatusCols]: any = await connection.execute(
            `SELECT COUNT(*) AS cnt FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'applications' AND COLUMN_NAME = 'status'`,
            [dbName]
        );
        ...
        try {
            ...
            const [misordered]: any = await connection.execute(`SELECT id, participant_a, participant_b FROM conversations WHERE participant_a > participant_b LIMIT 500`);
            for (const row of misordered) {
                const a = row.participant_a as string;
                const b = row.participant_b as string;
                const canonicalA = b < a ? b : a; // since a > b, canonicalA = b
                const canonicalB = b < a ? a : b;
                // Check if canonical conversation already exists
                const [existingCanonical]: any = await connection.execute(
                    `SELECT id FROM conversations WHERE participant_a = ? AND participant_b = ? LIMIT 1`,
            ...
            if (enforceAppId) {
                const [nullCheck1]: any = await connection.execute(`SELECT COUNT(*) AS cnt FROM conversations WHERE application_id IS NULL`);
                const [nullCheck2]: any = await connection.execute(`SELECT COUNT(*) AS cnt FROM messages WHERE application_id IS NULL`);
                if (nullCheck1[0].cnt === 0 && nullCheck2[0].cnt === 0) {
                    // Only alter if column currently nullable
                    const [colInfoConv]: any = await connection.execute(`SELECT IS_NULLABLE FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'conversations' AND COLUMN_NAME = 'application_id'`, [dbName]);
                    const [colInfoMsg]: any = await connection.execute(`SELECT IS_NULLABLE FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'messages' AND COLUMN_NAME = 'application_id'`, [dbName]);
            ...
            const [convUniqueRows]: any = await connection.execute(
                `SELECT COUNT(1) AS cnt FROM information_schema.STATISTICS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'conversations' AND INDEX_NAME = 'uniq_conv_participants'`,
                [dbName]
            );
